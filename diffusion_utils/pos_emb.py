...
def apply_rotary_emb(q: Tensor, k: Tensor, pos_emb: Tensor) -> Tuple[Tensor, Tensor]:
    sin, cos = pos_emb
...
class AxialRotaryEmbedding(nn.Module):
    ...
    def forward(self, x: Tensor) -> Tuple[Tensor, Tensor]:
        ...
        seq_x = torch.linspace(-1.0, 1.0, steps=h, device=device, dtype=dtype)
        seq_x = seq_x.unsqueeze(-1)

        seq_y = torch.linspace(-1.0, 1.0, steps=w, device=device, dtype=dtype)
